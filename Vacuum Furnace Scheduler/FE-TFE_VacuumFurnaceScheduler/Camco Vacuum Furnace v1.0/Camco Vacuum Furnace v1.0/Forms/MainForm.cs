...
namespace Vacuum_Furnace_Scheduler_v1._0
{
    public partial class MainForm : Form
    {
        ...
        private bool CheckBatchTable(DataTable bTable)
        {
            ...
            for (int i = 1; i < bTable.Rows.Count; i++)
            {
                if ((!bTable.Rows[i].Field<String>("Area").Equals(areaPrev) && !(bTable.Rows[i].Field<String>("Material").Equals("Carbon")))
                    || !(bTable.Rows[i].Field<String>("Material").Equals(matPrev)))
                    return false;
            }
            return true;
        }
